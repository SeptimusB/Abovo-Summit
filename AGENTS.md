...
- Treat `Library/Blank BP v26_0001.xlsb` as the authoritative unpopulated master and `Library/Demo BP v26_0001.xlsb` as the authoritative pre-populated master. Debug runs auto-load the repository Demo master. Both files retain the fully generated calculation formulas and disabled Formula Generation VBA; the Blank intentionally clears fourteen unlocked dummy/input formulas. `Library/TestFileClean.xlsb` is retained only as the preceding contract baseline. Do not infer workbook parity from similarly named VB methods alone.
