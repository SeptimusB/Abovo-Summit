# Historical schema-10 Abovo Summit project scope audit

> Superseded on 22 August 2026 when `SampleTestFileXLSB.xlsb` became the authoritative master. This file describes the retired ChatGPT-generated schema-10 workbook and migration implementation only.
...
- Version 7.20 extends Outputs with children 22-31 under Covenants and Other Reports and Other Cashflow Outputs. The ten read-only horizontal LiveGrid interfaces resolve nine workbook Outputs_* multi-area names plus Value for Money Metrics A9:M171, preserve workbook DisplayText/formatting, refresh after calculation, and target the matching worksheets. Scenario Planning is intentionally absent because Stress Test owns those interfaces. Macro-disabled read-only validation left TestFileMigrated.xlsb byte-identical at SHA-256 7039BD6E7BAE82C0269F1F9E56D4C28E17EED7599FFB263169798D214AABF044. XML validation confirmed one Outputs group, 32 unique children, exactly ten new children and no Scenario Planning group. Full standard Debug and Release rebuilds passed on 22 August 2026 with zero warnings and zero errors.
- Version 7.21 adds Outputs child 32, Analysis, as a worksheet-free special element routed directly to the existing BPIncomeExpenditureAnalyser class. It deliberately defines no DefaultWorksheet or workbook interface range, and the analyser class remains unchanged pending any future replacement-class work. XML validation confirmed one Outputs group, 33 unique children, exactly one Analysis child and no Scenario Planning group. Full standard Debug and Release rebuilds passed on 22 August 2026 with zero warnings and zero errors.
- Version 7.22 preserves the original BPIncomeExpenditureAnalyser as worksheet-free Analysis V1 and adds the independent corrected BPIncomeExpenditureAnalyserV2 as Analysis V2. V2 fixes Balance Sheet grouping/tab/export behavior, replaces helper and period ordinals with workbook field/header discovery, removes duplicate and unsafe paint behavior, explicitly enables read-only cell copy/multiselect, and owns deterministic RangeDataSource/calculation cleanup. Transactional DB structural synchronization now disconnects and reconnects both open versions. XML validation confirmed one Outputs group, 34 unique children, Analysis V1/V2 at CSIDs 32/33 and no Scenario Planning group. Full standard Debug and Release rebuilds passed on 22 August 2026 with zero warnings and zero errors.
- Version 7.23 introduces a model-scoped exclusive resource registry. Analysis V1 and V2 use one Transactional_Records RangeDataSource ownership key, so opening either version first disconnects, unregisters and disposes the other analyser and its document before constructing a replacement binding. CalcEngine can remove an active consumer by object identity, and model shutdown releases all registered owners before workbook services. This prevents two live DevExpress RangeDataSource instances from claiming the same cell range. The focused registry behavior test and full standard Debug and Release rebuilds passed on 22 August 2026 with zero warnings and zero errors.
- Version 7.24 corrects Analysis V2 period discovery after runtime testing exposed a stripped regular-expression escape. V2 recognizes year headings from generated grid field/caption metadata and falls back to the authoritative first row of Transactional_Records when DevExpress normalizes that metadata. The built matcher behavior test and full standard Debug and Release rebuilds passed with zero warnings and zero errors.
- Version 7.25 replaces every active direct Int32 conversion of Analysis V2 group summaries with one guarded parser. Numeric objects and culture-aware numeric strings remain usable, while blank, textual, non-finite and overflowing legacy values are ignored, preventing the interface-opening FormatException without changing Analysis V1 or the authoritative Transactional_Records workbook range. The focused parser test and full standard Debug and Release rebuilds passed on 22 August 2026 with zero warnings and zero errors.
