...
- The editable native Planner lists Test/Scenario 1 through 10 only. UI selection zero maps to workbook scenario index 1 (`Assumptions1`/`AssumptionsA1`, columns `I:M`), and later selections advance in five-column blocks through `Assumptions10`. The hidden `D:H` block and `S0Data` are the Base Case dashboard/captured result, not an editable Planner scenario. Dashboard and comparative selectors may still expose Base Case.
