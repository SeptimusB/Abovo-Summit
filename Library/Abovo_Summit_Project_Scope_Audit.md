...
- First-tab stresses and mitigation grids now capture the workbook value before the bound data source commits, restore it, and submit the edit through `ModelChangeManager`. This covers the main stresses, mitigation, development-mitigation, and money-mitigation ranges while retaining workbook lock enforcement.
...
- Planner presentation must not use `Cell.FillColor` alone: DevExpress flattens patterned conditional formats to their base grey through that property. The native renderer now uses the evaluated `Fill.PatternType`, displaying `Solid` cells with their background colour and patterned/empty locked cells with their evaluated pattern colour or white. Representative workbook verification produced `I10=Solid/editable/grey`, `J10=DarkGray/read-only/white`, `I35=Solid/editable/blue`, and `J35=None/read-only/white`.
- Scenario comparison must use the mapped source block: Base Case is `D:H` (hidden in the worksheet view), Scenario 1/Test 1 is `I:M`, and subsequent scenarios advance in five-column blocks. The visible worksheet screenshot of Test 1 is therefore compared with Scenario 1 in Summit, not Base Case.
