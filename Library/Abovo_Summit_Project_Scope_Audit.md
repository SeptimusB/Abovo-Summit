...
- Stress Test version 6.64 replaces the former Tab #4 chart wall with a workbook-driven Multivariable Dashboard prototype. It mirrors the ten-row calculated year window, the five covenant blocks and percentile signals, the workbook scenario/year/sort/graph control cells, the five black/red/target chart-source blocks, and the selected covenant breach timeline. The full worksheet/VBA mapping and staged fidelity plan are retained in `Multivariable_Dashboard_Technical_Audit.md`.
