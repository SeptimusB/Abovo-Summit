...
- The Tab #3 Test band header is tall enough to contain all three editor rows above the data-column captions. Custom-drawn workbook-linked editors explicitly transfer their repository appearance into the DevExpress painter, preserving the source workbook's resolved fill and font colours; standalone Copy From selectors use Abovo blue with white text, including their popup lists.
