...
Debug builds now auto-open the authoritative `Z:\Sandbox\SampleTestFileXLSB.xlsb` when that file exists. The hook is compile-time excluded from Release and still uses the normal model-open path.
