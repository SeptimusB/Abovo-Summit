...
7. **Client UI and recovery acceptance.** Funding keyboard navigation and month-end editor; DIT Save/Save As and clean-state button; sidebar compact/restore strips; normal/5k/mixed-DPI icons, fonts and restored layouts. P016's client-agreed Enter/Shift+Enter traversal is still an implementation task, not just acceptance: default horizontal, Down-selected vertical, next-grid/input and final wrap, with reverse traversal and scrolling. The shared navigator currently excludes Enter; its direction-memory clarification and test gates are recorded in the checkpoint report. Test recovery enable/disable, scheduling, notice/completion/failure, newer-copy prompt, read-only prior history, guided XLSB Save As, locked/network destinations and multiple models. Existing engineered/native tests have passed for their stated scope; they do not cover the new Enter contract or imply client-environment acceptance.
