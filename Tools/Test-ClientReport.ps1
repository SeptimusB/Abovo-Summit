param([string]$Configuration='Release', [string]$Workbook=(Join-Path (Split-Path -Parent $PSScriptRoot) 'Library/Demo BP v26_0001.xlsb'), [switch]$Inspect)
...
try{$compiled=$provider.CompileAssemblyFromSource($compiler,(Get-Content (Join-Path $PSScriptRoot 'ClientReportFixture.cs') -Raw));if($compiled.Errors.HasErrors){throw ($compiled.Errors | Out-String)}}finally{$provider.Dispose()}
...
try{& $runner $bin $out $Workbook ([string]$Inspect);if($LASTEXITCODE -ne 0){throw 'Client report fixture failed'}}finally{if((Get-FileHash -LiteralPath $Workbook).Hash -ne $hash){throw 'Source workbook changed'};Write-Output 'Source hash unchanged'}
