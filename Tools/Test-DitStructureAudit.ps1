...
$out=Join-Path $repo ('obj/DitStructureAudit/'+[guid]::NewGuid().ToString('N'))
...
Add-Type -TypeDefinition (Get-Content (Join-Path $PSScriptRoot 'DitStructureAudit.cs') -Raw) -OutputAssembly $runner -OutputType ConsoleApplication -ReferencedAssemblies $refs
...
try {& $runner $bin $Workbook $out; if($LASTEXITCODE -ne 0){throw 'DIT audit fixture failed'}}
