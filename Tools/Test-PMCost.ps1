...
$out=Join-Path $repo ('obj/PMCostTests/'+[guid]::NewGuid().ToString('N'))
...
try{
 $compiled=$provider.CompileAssemblyFromSource($compiler,(Get-Content -LiteralPath (Join-Path $PSScriptRoot 'PMCostFixture.cs') -Raw))
...
try{
 if($Workbook){& $runner $bin $out $source}else{& $runner $bin $out}
 if($LASTEXITCODE -ne 0){throw 'PMCost fixture failed'}
}finally{
 if($Workbook -and (Get-FileHash -LiteralPath $source).Hash -ne $hash){throw 'Source workbook changed'}
}
