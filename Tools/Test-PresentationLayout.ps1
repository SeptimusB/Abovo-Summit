param([string]$Configuration = 'Release', [string]$Workbook = 'C:\Repos\Abovo Summit\Library\Demo BP v26_0001.xlsb', [switch]$StressOnly, [switch]$GroupOnly, [switch]$SidebarOnly, [switch]$PanelsOnly)
...
& $runner $bin $Workbook $output $(if ($StressOnly) { '--stress-only' } elseif ($PanelsOnly) { '--panels-only' } elseif ($SidebarOnly) { '--sidebar-only' } elseif ($GroupOnly) { '--group-only' } else { '--all' })
