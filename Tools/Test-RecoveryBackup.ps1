param([string]$Configuration='Release',[string]$Workbook,[switch]$Probe,[string]$CompareWith,[switch]$VerifyDigests,[string]$CompareSheet,[switch]$CompareValues,
 [ValidateRange(0,100)][int]$FundingRecords=0,[ValidateSet('x86','x64')][string]$Architecture='x86')
$ErrorActionPreference='Stop'
if($FundingRecords -gt 0 -and (!$Workbook -or $Probe -or $CompareWith -or $VerifyDigests -or $CompareSheet -or $CompareValues)){throw 'Funding recovery requires a workbook and cannot be combined with comparison/probe modes'}
