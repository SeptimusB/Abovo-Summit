...
$out=Join-Path $repo ('obj/StructuralOrderAudit/'+[guid]::NewGuid().ToString('N'))
...
Add-Type -TypeDefinition (Get-Content (Join-Path $PSScriptRoot 'StructuralOrderAudit.cs') -Raw) -OutputAssembly $runner -OutputType ConsoleApplication -ReferencedAssemblies $refs
...
try {& $runner $bin $Workbook $out; if($LASTEXITCODE -ne 0){throw 'Structural order audit failed'}}
