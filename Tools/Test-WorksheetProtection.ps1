...
$out=Join-Path $repo ('obj/WorksheetProtectionTests/'+[guid]::NewGuid().ToString('N'))
[void](New-Item -ItemType Directory -Path $out)
$refs=@('System.Core','System.Drawing','System.Data','Microsoft.CSharp')
...
Add-Type -TypeDefinition (Get-Content (Join-Path $PSScriptRoot 'WorksheetProtectionFixture.cs') -Raw) -OutputAssembly $runner -OutputType ConsoleApplication -ReferencedAssemblies $refs
Copy-Item -LiteralPath (Join-Path $bin 'Abovo-summit.exe.config') -Destination ($runner+'.config')
Write-Output ('Output: '+$out)
& $runner $bin $out
if($LASTEXITCODE -ne 0){throw 'Worksheet protection fixture failed'}
