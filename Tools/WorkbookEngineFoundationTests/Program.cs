...
static class Program
{
    ...
    static async Task Run(string[] args)
    {
        ...
        if(args[2]=="consumers") { await NativeConsumerTests.Run(args[1]);Check(Hash(args[1])==before,"source bytes unchanged");return; }
        if(args[2]=="ownership") { await ExcelOwnershipTests.Run();Check(Hash(args[1])==before,"source bytes unchanged");return; }
        if(args[2]=="dashboard-native") { await NativeDashboardTests.Run(args[1]);Check(Hash(args[1])==before,"source bytes unchanged");return; }
        if(args[2]=="selection") { await EngineSelectionTests.Run(args[1]);Check(Hash(args[1])==before,"source bytes unchanged");return; }
        if(args[2]=="dit-opening-excel") { await NativeDitTests.Run(args[1],true,true,true);Check(Hash(args[1])==before,"source bytes unchanged");return; }
        if(args[2]=="ffr-native") { await NativeFfrTests.Run(args[1]);Check(Hash(args[1])==before,"source bytes unchanged");return; }
        ...
        if(args[2]=="range") { await RangeBindingTests.Run(args[1]);Check(Hash(args[1])==before,"source bytes unchanged");return; }
        if(args[2]=="presentation") { await PresentationTests.Run(args[1]);Check(Hash(args[1])==before,"source bytes unchanged");return; }
        if(args[2]=="bridge") { await EngineChangeManagerTests.Run(args[1]);Check(Hash(args[1])==before,"source bytes unchanged");return; }
        if(args[2]=="batch") { await BatchEditTests.Run(args[1]);Check(Hash(args[1])==before,"source bytes unchanged");return; }
